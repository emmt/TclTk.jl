...
interp = tk_start()
w = ".ttkstyle"
interp.eval(Nothing, "catch {destroy $w}")
top = TkToplevel(interp, w)
interp(Nothing, :wm, :title, top, "Simple Ttk Widgets")
interp(Nothing, :wm, :iconname, top, "ttkstyle")
...
function setState(interp::TclInterp, rootWidget, exceptThese, value)
    ...
    interp.eval(Nothing, "catch {$rootWidget state $value}")
...
TclTk.pack(Nothing, e, sep1, c1, c2, sep2, c3, c4, fill=:x, pady=2)
...
TclTk.pack(Nothing, r1, r2, r3, r4, r5, fill=:x, padx=3, pady=2)

## Arrange things neatly
f = TtkFrame(top)
f.pack(Nothing, fill="both", expand=true)
interp(Nothing, :lower, f)
TclTk.grid(Nothing, buttons, checks, radios, in=f, sticky="nwe", pady=2, padx=3)
TclTk.grid(Nothing, :columnconfigure, f, (0, 1, 2), weight=1, uniform=true)
